...
fn main() {
    let s: Box<&str> = "hello, world".into(); //& added
    greetings(s)
}
...
fn main() {
    let s = "hello, world".to_String(); // or String::from() can also be used
    greetings(s)
}
...
fn main() {
    ...
    let s1: &str = &s;  // or another method is to not convert s to_string() and pass it directly
